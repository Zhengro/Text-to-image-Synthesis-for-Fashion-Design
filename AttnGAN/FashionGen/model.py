...
os.environ['TORCH_MODEL_ZOO'] = '.local/AttnGAN/code/temp_model'
...
#         self.encoder = nn.Embedding.from_pretrained(weight)
    ...
    def forward(self, captions, cap_lens, hidden, mask=None):
        ...
        # output --> (batch, n_steps, hidden_size * num_directions)
        # output = self.drop(output)
        words_emb = output.transpose(1, 2)
        # words_emb --> batch x hidden_size*num_directions x n_steps
        if self.rnn_type == 'LSTM':
            sent_emb = hidden[0].transpose(0, 1).contiguous()
        else:
            sent_emb = hidden.transpose(0, 1).contiguous()
        sent_emb = sent_emb.view(-1, self.nhidden * self.num_directions)
        # sent_emb --> batch x num_directions*hidden_size
