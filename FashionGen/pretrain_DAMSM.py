...
def evaluate(dataloader, cnn_model, rnn_model, batch_size, labels, val_len):
    cnn_model.eval()
    rnn_model.eval()
    s_total_loss = 0
    w_total_loss = 0
    for step, data in enumerate(dataloader, 0):
        ...
        if step == len(dataloader)-1:
          batch_size = val_len-(len(dataloader)-1)*batch_size
      ...
      for epoch in range(start_epoch, cfg.TRAIN.MAX_EPOCH):
          ...
          if len(dataloader_val) > 0:
            s_loss, w_loss = evaluate(dataloader_val, image_encoder,
                                      text_encoder, batch_size, labels, len(dataset_val))
